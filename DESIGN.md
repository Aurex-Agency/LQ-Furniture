...
**The Slogan Headline.** The client-supplied "Never pay retail again!" artwork (`public/brand/never-pay-retail.png`, black brush caps with a white outline and green offset) IS the home hero h1, by client direction. It renders as an image inside the `h1` (alt "Never pay retail again!") at `max-w-2xl`, hoisting in through the same `.line-mask` entrance the old display lines used. It must sit above the hero scrims, never be recolored or rebuilt in type, and never repeat elsewhere without a client ask. "Limited Quantities + Unlimited Savings" remains the tagline in the footer, metadata, and JSON-LD.
